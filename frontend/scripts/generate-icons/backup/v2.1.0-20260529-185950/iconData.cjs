...
function getAllTasks() {
  ...
  uiButtons.forEach(btn => {
    tasks.push({
      type: 'ui_button',
      id: btn.name,
      filename: `ui_button_${btn.name}.png`,
  ...
  uiPanels.forEach(panel => {
    tasks.push({
      type: 'ui_panel',
      id: panel.name,
      filename: `ui_panel_${panel.name}.png`,
      outputDir: 'ui/panels',
      data: panel
    });
  });
