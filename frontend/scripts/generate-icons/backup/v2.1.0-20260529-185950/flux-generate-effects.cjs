 ...
 * 日期：2026-05-28
 * 版本：v1.0.0
 * 功能描述：FLUX开心农场特效作物生成工具——调用硅基流动FLUX.1-Kontext-dev API生成稀有/顶级作物生长阶段图和成品图标
 * 更新记录：
 ...
 *   3. 选项:
 *      --type rare      仅生成稀有作物(35-74, 240张)
 *      --type top       仅生成顶级作物(75-84, 60张)
 *      --type stages    仅生成生长阶段图(250张)
 *      --type products  仅生成成品图标(50张)
 *      --type single    生成单张测试图
 *      --id <数字>      指定作物ID (配合--type使用)
 *      --stage <1-5>    指定生长阶段 (配合--type stages使用)
 *      --dry-run        预览所有prompt但不生成
 *
 * 总计: 50种作物 × (5阶段 + 1成品) = 300张图片
 */
...
const { buildFluxPrompt, getDefaultParams } = require("./flux-prompt-builder.cjs");
...
function loadEnvConfig() {
  ...
  return {
    ...
    model:
      process.env.FLUX_MODEL || "black-forest-labs/FLUX.1-Kontext-dev",
    imageSize: process.env.IMAGE_SIZE || "128x128",
    numInferenceSteps:
      parseInt(process.env.FLUX_NUM_INFERENCE_STEPS) || 35,
    guidanceScale:
      parseFloat(process.env.FLUX_GUIDANCE_SCALE) || 8.0,
    outputDir:
      process.env.OUTPUT_DIR || "frontend/public/assets",
    maxConcurrent:
      parseInt(process.env.FLUX_MAX_CONCURRENT) || 2,
    requestDelayMs:
      parseInt(process.env.FLUX_REQUEST_DELAY_MS) || 3000
  };
}
...
function printHelp() {
  console.log(`
\x1b[36mFLUX 开心农场特效作物生成工具\x1b[0m
====================================
...
示例:
  node flux-generate-effects.cjs --type top                # 生成全部顶级作物
  node flux-generate-effects.cjs --type rare --id 55       # 生成天使果全部图片
  node flux-generate-effects.cjs --type stages --id 75 --stage 5  # 创世纪果阶段5
  node flux-generate-effects.cjs --type products --id 84    # 永恒圣果成品图标
  node flux-generate-effects.cjs --dry-run                  # 预览全部prompt
...
async function generateFluxIcon(config, prompt, task, retries = 3) {
...
async function processTasks(tasks, config, args) {
  ...
  console.log(`\n\x1b[36m开始生成 ${total} 张FLUX特效作物图片...\x1b[0m\n`);
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    try {
      ...
      const imageData = await generateFluxIcon(config, prompt, task);
  ...
  console.log(`\n\x1b[36m═══════════════════════════════════\x1b[0m`);
  console.log(`\x1b[36mFLUX特效作物生成完成!\x1b[0m`);
...
function printSummary(tasks) {
  const stageTasks = tasks.filter((t) => t.type === "growth_stage");
  const productTasks = tasks.filter((t) => t.type === "product_icon");

  const cropIds = [...new Set(tasks.map((t) => t.id))].sort((a, b) => a - b);
  const rareCount = cropIds.filter((id) => id >= 35 && id <= 74).length;
  const topCount = cropIds.filter((id) => id >= 75 && id <= 84).length;

  console.log(`  特效作物: ${cropIds.length} 种 (稀有 ${rareCount} + 顶级 ${topCount})`);
  console.log(`  生长阶段: ${stageTasks.length} 张 (5阶段 × ${cropIds.length}种)`);
  console.log(`  成品图标: ${productTasks.length} 张 (1张 × ${cropIds.length}种)`);
...
async function main() {
  ...
  console.log("\x1b[36m║  FLUX 开心农场特效作物生成工具 v1.0   ║\x1b[0m");
  console.log("\x1b[36m╚══════════════════════════════════════════╝\x1b[0m");
