  ...
  if (existing.rows.length > 0) {
    ...
    await db.query(
      ...
      [
        ...
        stats.reward_claimed_count,
        stats.conversion_rate,
        stats.day1_retention,
        stats.day7_retention,
        stats.event_id,
        stats.stat_date,
      ]
    );
...
const computeAllEventsStats = async (date = null) => {
  ...
  const events = await db.query(
    'SELECT id FROM game_events WHERE is_active = true'
  );
