...
exports.getShopGoodsList = async function (req, res) {
  try {
    ...
    const goods = await adminShopService.getShopGoodsList({
      goodsType,
      search,
      isOnSale,
    });
...
exports.updateShopGoods = async function (req, res) {
  try {
    ...
    const { goodsId } = req.params;
    const goods = await adminShopService.updateShopGoods(
      parseInt(goodsId),
      req.body
    );
...
exports.toggleShopGoodsStatus = async function (req, res) {
  try {
    ...
    const goods = await adminShopService.toggleShopGoodsStatus(
      parseInt(goodsId),
      isOnSale
    );
    res
      .status(200)
      .json({ success: true, data: goods, message: '状态更新成功' });
  } catch (error) {
