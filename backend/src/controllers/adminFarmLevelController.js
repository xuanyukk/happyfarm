...
exports.getFarmLevelDetail = async function (req, res) {
  try {
    ...
    const level = await adminFarmLevelService.getFarmLevelDetail(parseInt(levelId));
...
exports.updateFarmLevel = async function (req, res) {
  try {
    ...
    const { levelId } = req.params;
    const level = await adminFarmLevelService.updateFarmLevel(parseInt(levelId), req.body);
...
exports.deleteFarmLevel = async function (req, res) {
  try {
    const { levelId } = req.params;
    const result = await adminFarmLevelService.deleteFarmLevel(parseInt(levelId));
...
exports.updateFarmLevelValidation = [
  body('levelName').optional().isString().notEmpty().withMessage('等级名称不能为空'),
  body('experienceRequired').optional().isInt({ min: 1 }).withMessage('经验要求至少为1'),
  body('maxLandCount').optional().isInt({ min: 1 }).withMessage('最大地块数至少为1')
];