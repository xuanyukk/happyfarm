...
// 打印环境变量信息（脱敏处理，不输出完整DATABASE_URL）
...
// 生成查询缓存键（使用可读格式，支持按表名失效）
function generateCacheKey(query, params, tableName) {
  const hash = crypto.createHash('sha256');
  hash.update(query + JSON.stringify(params || []));
  const prefix = tableName ? `query:${tableName}:` : 'query:unknown:';
...
function extractTableName(query) {
  ...
  // 匹配 FROM table_name 或 JOIN table_name 模式
...
const cacheManager = {
  ...
  // 缓存键格式为 query:tableName:hash，因此可按表名前缀精确匹配
...
pool.query = async function (...args) {
  ...
  if (shouldCache) {
    ...
    const cacheKey = generateCacheKey(query, params, tableName);
