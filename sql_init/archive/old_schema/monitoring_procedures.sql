...
BEGIN
    -- 获取监控阈值
    SELECT CAST(config_value AS DECIMAL(10,2)) INTO threshold FROM system_config WHERE config_key = 'monitor_threshold';
...
BEGIN
    -- 获取每日最大获得限制
    SELECT CAST(config_value AS BIGINT) INTO max_daily_earn FROM system_config WHERE config_key = 'max_daily_earn';
