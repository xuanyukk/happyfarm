-- ============================================
-- 文件名: 20260606000002_log_cleanup_ttl.sql
-- 作者: 开发者
-- 日期: 2026-06-06
-- 版本: v1.0.0
-- 功能描述: 日志表自动清理策略，按 TTL 定期归档和删除历史数据
--           包含归档表创建、归档函数、清理函数和一站式维护函数
-- 依赖关系: 必须先执行 20260606000001_log_tables_partitioning.sql
--           确保 player_currency_log 和 game_activity_log 已转为分区表
-- 适用版本: PostgreSQL 12+
...
BEGIN
    ...
    LOOP
            ...
            IF moved_count > 0 THEN
                ...
                IF NOT EXISTS (
                    ...
                    EXECUTE format(
                        ...
                        partition_name, archive_table, partition_start, partition_end
                    );
                END IF;

                -- 移动数据到归档表
                EXECUTE format(
                    'INSERT INTO %I SELECT * FROM %I WHERE create_time < %L',
                    partition_name, partition_record.partition_name, cutoff_date
                );
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '归档分区 % 失败: %', partition_record.partition_name, SQLERRM;
        END;
    END LOOP;
    ...
    LOOP
        BEGIN
            ...
            IF row_count = 0 THEN
                ...
                RAISE NOTICE '已删除空分区: %', partition_record.partition_name;
            END IF;
        EXCEPTION WHEN OTHERS THEN
            RAISE WARNING '清理分区 % 失败: %', partition_record.partition_name, SQLERRM;
        END;
    END LOOP;
...
BEGIN
    ...
    archived_count := archive_old_logs('player_currency_log', 'player_currency_log_archive', 3);
    RAISE NOTICE '-- 归档 game_activity_log --';
    archived_count := archived_count + archive_old_logs('game_activity_log', 'game_activity_log_archive', 3);

    -- 2. 清理超过6个月的空分区
    RAISE NOTICE '-- 清理空分区 --';
    cleaned_count := cleanup_empty_partitions('player_currency_log', 6);
    cleaned_count := cleaned_count + cleanup_empty_partitions('game_activity_log', 6);
    ...
    RAISE NOTICE '归档: % 条, 清理: % 个分区', archived_count, cleaned_count;
END;
$$ LANGUAGE plpgsql;
